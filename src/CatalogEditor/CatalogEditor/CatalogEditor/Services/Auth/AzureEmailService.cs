...
public class AzureEmailService(IOptions<AzureEmailOptions> options, ILogger<AzureEmailService> logger) : IEmailService
{
    public async Task SendVerificationCodeAsync(string email, string code)
    {
        ...
        var message = new EmailMessage(
            ...
            content: new EmailContent("Your Catalog Editor sign-in code")
            {
                PlainText = $"Your sign-in code is: {code}\n\nThis code expires in 15 minutes.",
                Html = $"<p>Your sign-in code is: <strong>{code}</strong></p><p>This code expires in 15 minutes.</p>"
            });
